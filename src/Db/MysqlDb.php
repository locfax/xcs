...
class MysqlDb extends Database
{
    ...
    public function remove(string $tableName, array|string $condition, array $args = [], bool $multi = false): bool|int
    {
        ...
        $condition = empty($condition) ? '' : ' WHERE ' . $condition;
        $limit = $multi ? '' : ' LIMIT 1';
        return $this->execSql(sprintf('DELETE FROM %s%s%s', $this->qTable($tableName), $condition, $limit), $args);
    }
    ...
    public function first(string $tableName, string $field, array|string $condition, array $args = [], string $orderBy = ''): mixed
    {
        ...
        $condition = empty($condition) ? '' : ' WHERE ' . $condition;
        $orderBy = empty($orderBy) ? '' : ' ORDER BY ' . $orderBy;
        return $this->firstSql(sprintf('SELECT %s FROM %s%s%s LIMIT 1', $field, $this->qTable($tableName), $condition, $orderBy), $args);
    }
