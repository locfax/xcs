...
class File
{
    ...
    public function get(string $key, bool $isPath = false): mixed
    {
        if ($isPath) {
            ...
            $cacheFile = CACHE_PATH . $this->filePath($hashKey) . DS . $config['prefix'] . '_' . $hashKey . '.php';
        }
     ...
     * @return bool
     */
    public function set(string $key, mixed $val, int $ttl = 0, bool $isPath = false): bool
    {
        if ($ttl > 0) {
            $timeout = time() + $ttl;
        } else {
        ...
        if ($isPath) {
            ...
            $cacheFile = $path . DS . $config['prefix'] . '_' . $hashKey . '.php';
        }
     ...
     * @return bool
     */
    public function rm(string $key, bool $isPath = false): bool
    {
        if ($isPath) {
            $cacheFile = APP_ROOT . DS . $key;
        } else {
     ...
     * @return bool
     */
    private function save($filename, $content, $mode): bool
    {
        if (!is_file($filename)) {
    ...
    private function filePath($hash): string
    {
        ...
        return $dir1 . DS . $dir2 . DS . $dir3;  // 拼接成文件路径
    }
